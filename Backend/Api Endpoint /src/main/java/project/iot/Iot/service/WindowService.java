...
public class WindowService {
    ...
    public WindowService(@Qualifier("sensorMongoTemplate") MongoTemplate sensorMongoTemplate)
    {
        this.sensorMongoTemplate=sensorMongoTemplate;
    }
     public Boolean getStatusWindow(String roomId) {
        Query query = new Query();
        query.addCriteria(Criteria.where("id").is(roomId));    
        Agent agent = sensorMongoTemplate.findOne(query, Agent.class);
        
        if (agent != null) {
            return agent.getWindow();
        }
        
        return null;
    }
    public Boolean toggleWindow(String roomId)
    {
        Query query = new Query();
        query.addCriteria(Criteria.where("id").is(roomId));
        Agent agent = sensorMongoTemplate.findOne(query, Agent.class);
        ...
        update.set("window", !agent.getWindow());
